...
const wrestlerRecords=wrestlers.map(wrestler=>{
    ...
    function outcome(result){
        ...
        if(
            ...
            scores.forEach((score,index)=>{
                if(score===highest)winnerIndexes.push(index);
            });

            const thisIndex=result.participants.findIndex(isThisWrestler);

            if(winnerIndexes.length!==1){
                return winnerIndexes.includes(thisIndex)?"WIN":"DRAW";
            }

            return isThisWrestler(result.participants[winnerIndexes[0]])?"WIN":"LOSS";
    ...
    function processResults(results){
        ...
        (results||[]).forEach(result=>{
            ...
            if(Array.isArray(result)){
                ...
                if(!isNaN(a)&&!isNaN(b)){
                    if(a>b)winner=wrestler1;
                    else if(b>a)winner=wrestler2;
                }
    ...
    Object.values(eventData||{}).forEach(event=>{
        (event.results||[]).forEach(addOutcome);
    });
    ...
    if(typeof tournamentData!=="undefined"){
        ...
        Object.values(tournamentData).forEach(data=>{

            if(!data)return;

            if(data.weekly){
                Object.values(data.weekly).forEach(event=>{
                    processResults(event.results||[]);
                });
            }

            if(data.matches&&!Array.isArray(data.matches)){
                Object.values(data.matches).forEach(event=>{
                    processResults(event.results||[]);
                });
            }

            if(data.shows){
                Object.values(data.shows).forEach(event=>{
                    processResults(event.matches||[]);
                });
            }
    ...
    if(typeof outsiderData!=="undefined"){
        ...
        Object.values(outsiderData).forEach(data=>{
            ...
            if(data.finalMatches){
                processResults(data.finalMatches);
            }
...
const filterButtons=document.querySelectorAll(".record-filter");
const pagesContainer=document.getElementById("record-pages");
...
function renderRecords(recordList){
    ...
    if(recordList.length===0){
        ...
        recordsContainer.innerHTML=`
            <div class="record-card">
                <div class="record-name">NO WRESTLERS FOUND</div>
            </div>
        `;
    ...
    const start=(currentPage-1)*wrestlersPerPage;
    const end=start+wrestlersPerPage;
    const pageRecords=recordList.slice(start,end);
    ...
    pageRecords.forEach(wrestler=>{
        ...
        card.innerHTML=`
            <div class="record-top">
                <div class="record-name">${wrestler.name}</div>
                <div class="record-brand">${wrestler.brand}</div>
            </div>
            <div class="record-result">
                <span class="record-label">RECORD</span>
                <span class="record-value">${wrestler.wins} - ${wrestler.draws} - ${wrestler.losses}</span>
            </div>
        `;

        card.addEventListener("click",()=>{
            window.location.href=`superstar.html?id=${wrestler.id}`;
        });
...
function getFilteredRecords(){
    ...
    return wrestlerRecords.filter(wrestler=>{
        ...
        const matchesSearch=wrestler.name.toLowerCase().includes(search);

        const matchesBrand=
            currentBrand==="ALL"||
            wrestler.brand.toUpperCase()===currentBrand;
...
function updateRecords(){
    ...
    const totalPages=Math.max(
        1,
        Math.ceil(filteredRecords.length/wrestlersPerPage)
    );
